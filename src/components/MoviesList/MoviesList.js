...
const MovieList = ({ handleSelect, sortProp, initialValue, setRef, isReset}) => {
  ...
  const NotFound = () => {
    return (
      <div className={s.notFound} ref={ref}>
  ...
  const sortMovies = () => {
    ...
    let indArr1 = valuesArr.map((item, ind) => item.length === 3 ? ind : null).filter(ind => ind !== null);
    let indArr2 = valuesArr.map((item, ind) => item.length === 2 ? ind : null).filter(ind => ind !== null);
    let indArr3 = valuesArr.map((item, ind) => item.length === 1 ? ind : null).filter(ind => ind !== null);
    ...
    if (sortParamss.filter(s => s).length === 0) {
      return data
    } else if (result1.length === 0 && sortParamss.filter(s => s).length === 3 || result2.length === 0 && sortParamss.filter(s => s).length === 2) {
      return [];
